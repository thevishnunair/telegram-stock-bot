...
async def handler(event):
    ...
    # Album of static images
    if hasattr(event, 'grouped_id') and event.grouped_id:
        album_messages = [
            m async for m in client.iter_messages(source_channel, min_id=message.id - 10, max_id=message.id + 10)
            if m.grouped_id == message.grouped_id
        ]
        images = []
        for m in album_messages:
            if isinstance(m.media, MessageMediaPhoto):
                images.append(m.media.photo)
            else:
                return  # Skip if any media is not a photo
        await client.send_file(destination_channel, images)
        return
